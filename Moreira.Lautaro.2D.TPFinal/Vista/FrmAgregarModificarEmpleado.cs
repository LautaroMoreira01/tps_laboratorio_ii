    ...
    public partial class FrmAgregarModificarEmpleado : Form
    {
        ...
        private void FrmAgregarModificarEmpleado_Load(object sender, EventArgs e)
        {
            ...
            if (empleado is not null)
            {
                ...
                tbEdad.Text = empleado.Edad.ToString();
        ...
        private void btnAgregarModificar_Click(object sender, EventArgs e)
        {
            try
            {
                if (ValidarCampos())
                {
                    ...
                    if(empleado is null)
                    {
                        empleado = new Empleado(tbNombre.Text, tbApellido.Text, dni, edad,
        ...
        /// <summary>
        /// valida si el campo edad es correcto
        /// </summary>
        /// <returns>true si el campo es correcto, si no lamza una excepcion de campo invalido</returns>
        /// <exception cref="CampoInvalidoException">excepcion a lanzar si el campo es invalido</exception>
        private bool ValidarCampoEdad()
        {
            int Edad;

            if (tbEdad.Text != String.Empty && int.TryParse(tbEdad.Text, out Edad))
            {
                return true;
            }
            else
            {
                throw new CampoInvalidoException("El campo edad no puede estar vacio y debe contener solamente numeros.");
            }
        }
